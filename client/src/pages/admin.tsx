    ...
    if (editingUser) {
      fetch("/api/admin/supabase/accounts")
        ...
        .then(async (data) => {
          ...
          const assigned = Array.isArray(data) ? data.find((a: any) => a.trader_username === editingUser.username && !String(a.trader_username).startsWith('available')) : null;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={() => setEditingUser(null)}>
          <div className="bg-card border border-b1 rounded-xl p-6 max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={async (e) => { 
              ...
              // Find current account number for this trader so we can reset it to "available X"
              const currentRow = mt5Accounts.find((a: any) => a.trader_username === editingUser.username);
              const currentAccountNum = currentRow?.mt5_account;

              if (!editingUser.mt5Account) {
                // Unassign — reset to "available {account_number}"
                if (currentAccountNum) {
                  await supabaseClient.from('accounts').update({ trader_username: `available ${currentAccountNum}` }).eq('trader_username', editingUser.username);
                }
              } else {
                // First reset old row back to available
                if (currentAccountNum) {
                  await supabaseClient.from('accounts').update({ trader_username: `available ${currentAccountNum}` }).eq('trader_username', editingUser.username);
                }
                // Then assign new row to this trader
                  ...
                  >
                    ...
                    {mt5Accounts.map((acc: any) => (
                      <option key={acc.mt5_account} value={acc.mt5_account}>
                        {(!acc.trader_username || String(acc.trader_username).startsWith('available')) ? `Available — ${acc.mt5_account}` : `${acc.trader_username} — ${acc.mt5_account}`}
                      </option>
                    ))}
                  </select>
